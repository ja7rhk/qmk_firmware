# SKYLOONG [GK61 Pro](https://skyloongtech.com/skyloong-gk61pro/ref/13/) 用 Single Nicola配列キーマップ
キーボード側でシングル親指シフトをローマ字(※英字キー入力から仮名への変換方式)出力するためのキーマップです。  
物理的なキー配置はANSI配列で、Windowsの設定では日本語/英語キーボード(101/102キー)配列で使用します。  
「」や『』・などは直接出力できませんから、MS-IMEなどの変換機能で確定します。

* 左ALTキー : 単押し → IME On 、長押し → TAB(変換候補)
* Spaceキー : 単押し → Spaceキー 、長押し → F14(無変換キー)

## 配列
![](gk61_pro_single_nicola_layout.png)
...
(1) 左ALTでMS-IME側"IME on"、CAPSLOCKで"IME off"に切り替える。
(2) "observe_ime"はIMEの状態からキーボードにNum Lockで通知する。  
(3) キーボードではNum Lockの状態に従ってNicolaモードをon/offする。  

※この機能を利用しない場合は\pro_48_ss\config,hの#define USE_OBSERVE_IMEをコメントアウトして下さい。  
